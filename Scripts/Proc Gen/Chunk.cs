...
public class Chunk : MonoBehaviour
{
    ...
    [SerializeField] float bottleSpawnChance = .05f;
    ...
    public void Init(LevelGenerator levelGenerator, ScoreManager scoreManager, PlayerCollisionHandler collisionHandler) 
    {
    ...
    void SpawnBottle() // Spawns bottle in much the same as apple
    {
        if (Random.value > bottleSpawnChance || availableLanes.Count <= 0 || collisionHandler == null) return;

        int selectedLane = SelectLane();

        Vector3 spawnPosition = new Vector3(lanes[selectedLane], transform.position.y, transform.position.z);
        Bottle newBottle = Instantiate(bottlePrefab, spawnPosition, Quaternion.identity, this.transform).GetComponent<Bottle>();
        newBottle.Init(collisionHandler);
    }
